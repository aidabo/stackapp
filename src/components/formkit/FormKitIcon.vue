<template>
  ...
  <Suspense>
    <FormKitProvider :config="FormKitConfig">
      <FormKit type="text" label="FormKit Input" :prefix-icon="icon" />
      <FormKitIcon :icon="icon" />
      <select v-model="icon">
        <option v-for="iconName in icons" :key="iconName" :value="iconName">
          {{ iconName }}
        </option>
      </select>
      ...
      />
      <h3>custom loader</h3>
      <FormKitIcon :icon-loader="fontAwesomeLoader" icon="lightbulb" />
    </FormKitProvider>
  </Suspense>
</template>
...
const fontAwesomeLoader = (iconName: any): any => {
  return fetch(
